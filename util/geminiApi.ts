...
async function getModelConfig(user: User) {
	return {
		...
		systemInstruction: [
			...
			'Ao longo da conversa, você deve escrever várias memórias sobre os interesses, preferências e características do usuário.',
			...
			'Modelo de Alarme: "{ALARM:humurous message:duration}"',
			...
			'Não referencie memórias e alarmes usando o modelo. Diga sobre o que se trata, mas não escreva {MEMORY:texto} e nem {ALARM:texto:1h} quando estiver referenciando uma memória ou alarme.',
			'Memórias do usuário:',
			...user.memories,
			'Alarmes do usuário:',
